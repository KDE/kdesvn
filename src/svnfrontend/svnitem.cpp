...
QPixmap SvnItem::getPixmap(const QPixmap &_p, int size, bool overlay)
{
    if (!isVersioned()) {
        ...
        if (mod) {
            m_bgColor = MODIFIED;
            if (overlay) {
                p2 = QIcon::fromTheme(QStringLiteral("kdesvnmodified")).pixmap(size);
            }
        }
        if (!p2.isNull()) {
            QPixmap p;
...
QPixmap SvnItem::getPixmap(int size, bool overlay)
{
    ...
    if (svn::Url::isValid(p_Item->m_Stat->path())) {
        /* remote access */
        p = QIcon::fromTheme(p_Item->mimeType(isDir()).iconName()).pixmap(size);
        if (isLocked()) {
            m_bgColor = LOCKED;
            if (overlay) {
                QPixmap p2 = QIcon::fromTheme(QStringLiteral("kdesvnlocked")).pixmap(size);
                if (!p2.isNull()) {
                    QImage i1;
                    i1 = p.toImage();
                    QImage i2;
        ...
        if (isRemoteAdded()) {
            if (isDir()) {
                p = QIcon::fromTheme(QStringLiteral("folder")).pixmap(size);
            } else {
                p = QIcon::fromTheme(QStringLiteral("unknown")).pixmap(size);
            }
        } else {
            // local access
            ...
            p = QIcon::fromTheme(mimeType.iconName()).pixmap(size);
...
{
    return getWrapper()->isUpdated(p_Item->m_Stat->path()) && p_Item->m_Stat->validReposStatus() && !p_Item->m_Stat->validLocalStatus();
}
...
{
    return p_Item->m_Stat->nodeStatus() == svn_wc_status_modified || p_Item->m_Stat->nodeStatus() == svn_wc_status_replaced;
}
