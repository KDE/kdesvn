...
Revision Client_impl::checkout(const CheckoutParameter &parameters)
{
    Pool subPool;
...
Revision Client_impl::remove(const Targets &targets, bool force, bool keep_local, const PropertiesMap &revProps)
{
    Pool pool;
    svn_error_t *error;
    ...
    error = svn_client_delete4(targets.array(pool), force, keep_local, map2hash(revProps, pool), commit_callback2, &_baton, *m_context, pool);
...
void Client_impl::revert(const Targets &targets, Depth depth, const StringArray &changelist)
{
    Pool pool;

    svn_error_t *error = svn_client_revert2((targets.array(pool)), internal::DepthToSvn(depth), changelist.array(pool), *m_context, pool);
    if (error != nullptr) {
        throw ClientException(error);
    }
}

void Client_impl::add(const Path &path, svn::Depth depth, bool force, bool no_ignore, bool add_parents)
{
    Pool pool;
...
Revisions Client_impl::update(const UpdateParameter &params)
{
    Pool pool;
    Revisions resulting;
    svn_error_t *error;
    ...
    apr_array_header_t *apr_revisions = apr_array_make(apr_pool, params.targets().size(), sizeof(svn_revnum_t));
    error = svn_client_update4(&apr_revisions,
                               params.targets().array(pool),
                               params.revision(),
                               internal::DepthToSvn(params.depth()),
                               params.sticky_depth(),
                               params.ignore_externals(),
                               params.allow_unversioned(),
                               params.add_as_modification(),
                               params.make_parents(),
                               *m_context,
                               pool);
    ...
    for (int i = 0; i < apr_revisions->nelts; ++i) {
        svn_revnum_t *_rev = &APR_ARRAY_IDX(apr_revisions, i, svn_revnum_t);
...
svn::Revision Client_impl::commit(const CommitParameter &parameters)
{
    Pool pool;
...
Revision Client_impl::copy(const CopyParameter &parameter)
{
...
Revision Client_impl::copy(const Path &srcPath, const Revision &srcRevision, const Path &destPath)
{
...
svn::Revision Client_impl::mkdir(const Targets &targets, const QString &msg, bool makeParent, const PropertiesMap &revProps)
{
    Pool pool;
    ...
    mBaton _baton;
    _baton.m_context = m_context;
    error = svn_client_mkdir4(const_cast<apr_array_header_t *>(targets.array(pool)),
                              makeParent,
...
void Client_impl::cleanup(const Path &path)
{
    Pool subPool;
...
Revision Client_impl::doExport(const CheckoutParameter &params)
{
    Pool pool;
    svn_revnum_t revnum = 0;
    QByteArray _neolBA;
...
Revision Client_impl::doSwitch(const Path &path,
                               const Url &url,
...
Revision Client_impl::import(const Path &path,
...
void Client_impl::relocate(const Path &path, const Url &from_url, const Url &to_url, bool recurse, bool ignore_externals)
{
    Q_UNUSED(recurse);
    Pool pool;
